...
{
    return ioctl(__uart_dev, UART_RXBUF_LEVEL, ch);		//(&ch여야하는거아닌가..?)  성공시 0, 에러시 -1
}
...
int uart_read(int ch, unsigned char *ubuf, int size)
{
    ...
    uart_d.uart_ch = ch;	//1
    uart_d.buf_size = size;		//0
    uart_d.uart_buf = ubuf;		//buf[0]의 주소
...
{
	int ret;
	//printf("uart_tx_buf_full(UART1) : %d\n", uart_tx_buf_full(UART1));
...
void uart1_buffer_read(unsigned char *buf, int size)
{
	...
	while(1) {
		rx_len = 0;
		rx_cnt += rx_len;
		rx_len = uart_rx_level(UART1);	//성공가정 0 대입	
		//printf("uart_rx_level(UART1) : %d\n", uart_rx_level(UART1));
		uart_read(UART1, &buf[rx_cnt], rx_len);
		//printf("uart_read(UART1, &buf[rx_cnt], rx_len) : %d\n", uart_read(UART1, &buf[rx_cnt], rx_len));
		rx_len += 1;
